...
- **Custom Memory Load:** 
  ...
  - Release an arbitrary number of GB at once via the "Release Multiple GB..." dialog.
- **Real-Time Stats:** 
  - Displays physical RAM and committed memory usage.
  - Shows number of active memory-consuming processes.
...
- Fixed a race condition that occurs when many consumers try to allocate large amounts of memory at once.
- Consumers can stop consuming and instantly release all memory.

### Sleep and Hibernation awareness
- When Windows enters a suspended state, all consumers will stop their activity and automatically resume when the system wakes up.
